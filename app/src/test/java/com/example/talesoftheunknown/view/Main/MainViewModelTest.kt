...
import com.example.talesoftheunknown.DataDummy
import com.example.talesoftheunknown.Injection
import com.example.talesoftheunknown.MainDispatcherRule
...
import com.example.talesoftheunknown.getOrAwaitValue
...
//        val data: PagingData<ListStoryItem> = PagingData.from(dummyStory)
        val data: PagingData<ListStoryItem> = StorySource.snapshot(dummyStory)
        val expectedStory = MutableLiveData<PagingData<ListStoryItem>>()
        expectedStory.value = data
        Mockito.`when`(storyRepository.getPagingStory()).thenReturn(expectedStory)
//        Mockito.`when`(repository.getSession()).thenReturn(null)
        val mainViewModel = MainViewModel(repository, storyRepository)
    ...
    fun `when Get Story Empty Should Return No Data`() = runTest {
        val dummyStory = DataDummy.generateDummyQuoteResponse()
//        val data: PagingData<ListStoryItem> = PagingData.from(dummyStory)
        val data: PagingData<ListStoryItem> = StorySource.snapshot(emptyList())
        val expectedStory = MutableLiveData<PagingData<ListStoryItem>>()
        expectedStory.value = data
        Mockito.`when`(storyRepository.getPagingStory()).thenReturn(expectedStory)
        val mainViewModel = MainViewModel(repository, storyRepository)
...
class StorySource : PagingSource<Int, LiveData<List<ListStoryItem>>>() {
    companion object {
